...
#import "RKMacros.h"
#import "AF_Networking_RestKit.h"
...
/**
 ...
 Each object manager is configured with a base URL that defines the URL that all request sent through the manager will be relative to. The base URL is configured directly through the `managerWithBaseURL:` method or is inherited from an AFNetworking `AF_HTTPClient_RestKit` object if the manager is initialized via the `initWithHTTPClient:` method. The base URL can point directly at the root of a URL or may include a path.
 ...
 Routing interfaces are provided by the `RKRouter` class. Each object manager is in initialized with an `RKRouter` object with a baseURL equal to the baseURL of the underlying `AF_HTTPClient_RestKit` object. Each `RKRouter` instance maintains an `RKRouteSet` object that manages a collection of `RKRoute` objects. Routes are defined in terms of a path pattern.
 ...
 * `enqueueObjectRequestOperation:` - Invoked to enqueue all operations constructed by the manager that are to be started as soon as possible. Provide a subclass implementation if you wish to work with object request operations as they are be enqueued.
 ...
 If you wish to more specifically customize the behavior of the lower level HTTP details, you have several options. All HTTP requests made by the `RKObjectManager` class are made with an instance of the `RKHTTPRequestOperation` class, which is a subclass of the `AF_HTTPRequestOperation_RestKit` class from AFNetworking. This operation class implements the `NSURLConnectionDelegate` and `NSURLConnectionDataDelegate` protocols and as such, has full access to all details of the HTTP request/response cycle exposed by `NSURLConnection`. You can provide the object manager with your own custom subclass of `RKHTTPRequestOperation` to the manager via the `setHTTPOperationClass:` method and all HTTP requests made through the manager will pass through your operation.

 You can also customize the HTTP details at the AFNetworking level by subclassing `AF_HTTPClient_RestKit` and using an instance of your subclassed client to initialize the manager.
 
 @warning Note that when subclassing `AF_HTTPClient_RestKit` to change object manager behaviors it is not possible to alter the paramters of requests that are constructed on behalf of the manager. This is because the object manager handles its own serialization and construction of the request body, but defers to the `AF_HTTPClient_RestKit` for all other details (such as default HTTP headers, etc).
...
/**
 Creates and returns a new `RKObjectManager` object initialized with a new `AF_HTTPClient_RestKit` object that was in turn initialized with the given base URL. The RestKit defaults are applied to the object manager.
 
 When initialized with a base URL, the returned object manager will have a `requestSerializationMIMEType` with the value of `RKMIMETypeFormURLEncoded` and the underlying `HTTPClient` will have a default value for the 'Accept' header set to `RKMIMETypeJSON`, and the `AF_JSONRequestOperation_RestKit` class will be registered.
 
 @param baseURL The base URL with which to initialize the `AF_HTTPClient_RestKit` object
 @return A new `RKObjectManager` initialized with an `AF_HTTPClient_RestKit` that was initialized with the given baseURL.
 */
 ...
 */
@property (nonatomic, strong, readwrite) AF_HTTPClient_RestKit *HTTPClient;
...
/**
 ...
 The returned dictionary contains all of the default headers set on the underlying `AF_HTTPClient_RestKit` object and the value of the 'Accept' header set on the object manager, if any.
 ...
 **Default**: `RKMIMETypeFormURLEncoded` or the value of the parameter encoding for the underlying `AF_HTTPClient_RestKit`.
 */
...
/**
 ...
 This method wraps the underlying `AF_HTTPClient_RestKit` method `multipartFormRequestWithMethod:path:parameters:constructingBodyWithBlock` and adds routing and object parameterization.
 ...
 @see [AF_HTTPClient_RestKit multipartFormRequestWithMethod:path:parameters:constructingBodyWithBlock]
 */
